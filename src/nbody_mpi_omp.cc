...
static unsigned long long seed_now() {
    auto now = std::chrono::high_resolution_clock::now();
    auto now_us = std::chrono::time_point_cast<std::chrono::microseconds>(now);
    return (unsigned long long)now_us.time_since_epoch().count();
}
...
static void initial_conditions(vector<double>& x, vector<double>& v) {
    ...
    // Hybrid seeding: different across ranks and threads
    unsigned long long base_seed = seed_now() ^ (unsigned long long)(rank * 0x9e3779b97f4a7c15ULL);

    #pragma omp parallel
    {
        int thread = omp_get_thread_num();
        std::mt19937 gen((unsigned int)(base_seed ^ (unsigned long long)(thread + 1) * 1234567ULL));
...
static void compute_accel_local(const vector<double>& x, const vector<double>& m, vector<double>& a_local) {
...
static void timestep(vector<double>& x, vector<double>& v, const vector<double>& m) {
    ...
    compute_accel_local(x, m, a_local);
...
int main(int argc, char** argv) {
    ...
    if (rank == 0) std::system("mkdir -p results reports reports/data");
    ...
    if (rank == 0) {
        std::cout << "Runtime = " << elapsed_max << " s for N = " << N
                  ...
                  << " and OMP_NUM_THREADS=" << omp_get_max_threads() << "\n";
        save_vec(KE_root, "results/ke_hybrid_N" + std::to_string(N) + ".txt", "KineticEnergy");
    }
